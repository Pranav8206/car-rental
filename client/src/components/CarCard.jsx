...
const CarCard = ({car}) => {
  ...
  return (
    ...
    transition={{duration: 0.3}}
     onClick={()=> {navigate(`/car-details/${car._id}`)}} className='group rounded-xl overflow-hidden shadow-lg hover:-translate-y-3 hover:-mx-0.5
    transition-all duration-500 cursor-pointer'>
        <div className='relative h-52 overflow-hidden'>
            ...
            <div className='absolute bottom-4 right-2 bg-black/80 backdrop-blur-sm text-white px-3 py-2 rounded-lg'>
                <span className='font-semibold'> {currency}{car.pricePerDay} </span>
                <span className='text-sm to-white/80'> / day</span>
            </div>
        </div>
